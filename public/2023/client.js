...
var AVATARS = 38; // jonah was 37
//the big file if used
var ALL_AVATARS_SHEET = "allAvatars2.png"; // jonah
...
function preload() {
    ...
    */
    ...
    var logoSheet = loadSpriteSheet(ASSETS_FOLDER + LOGO_FILE, 66, 90, 4);
...
function newGame() {
    ...
    socket.on('playerChanged',
        function (p) {
            try {               
                console.log("updated character: " + p.avatar);
              
                if (players.hasOwnProperty(p.id)) {
                    players[p.id].avatar = p.avatar;
                    players[p.id].updateCharacter();
                }
            } catch (e) {
                console.log("Error on playerChanged");
                console.error(e);
            }
        }
    );
...
function avatarSelection() {
    ...
    previousBody.onMouseReleased = function () {
        currentAvatar -= 1;
        if (currentAvatar < 0)
            currentAvatar = AVATARS - 2; // jonah was -1
    ...
    nextBody.onMouseReleased = function () {
        currentAvatar += 1;
        if (currentAvatar >= AVATARS - 1) // jonah was no -1
            currentAvatar = 0;
...
function Player(p) {
    ...
    this.updateCharacter = function () {
        //tint the image
        this.avatarGraphics = paletteSwap(walkSheets[this.avatar], AVATAR_PALETTES_RGB[this.color], this.tint);
        this.spriteSheet = loadSpriteSheet(this.avatarGraphics, AVATAR_W, AVATAR_H, round(walkSheets[this.avatar].width / AVATAR_W));
        this.walkAnimation = loadAnimation(this.spriteSheet);
        //emote
        this.emoteGraphics = paletteSwap(emoteSheets[this.avatar], AVATAR_PALETTES_RGB[this.color], this.tint);
        this.emoteSheet = loadSpriteSheet(this.emoteGraphics, AVATAR_W, AVATAR_H, round(emoteSheets[this.avatar].width / AVATAR_W));
        this.emoteAnimation = loadAnimation(this.emoteSheet);
        this.emoteAnimation.frameDelay = 10;

        //this.sprite = createSprite(100, 100);

        //this.sprite.scale = ROOMS[this.room].avatarScale;
...
function keyPressed() {
    ...
    // link to trailer
    if (nickName != "" && screen == "game" && longText != "" && longText != SETTINGS.INTRO_TEXT && keyCode == 32) {
      if (longTextLink != "") {
        window.open(longTextLink, '_blank');
      }
    }
...
function randomAvatar() {
    currentColor = floor(random(0, AVATAR_PALETTES.length));
    currentAvatar = floor(random(0, AVATARS - 1)); // jonah no -1
    previewAvatar();
}
