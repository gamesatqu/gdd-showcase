...
var PACKETS_PER_SECONDS = 100; // jonah was 30
...
io.on('connection', function (socket) {
            ...
            if (time - gameState.players[socket.id].lastMessage > DATA.SETTINGS.ANTI_SPAM && !gameState.players[socket.id].muted) {
                ...
                if (gameState.players[socket.id].admin && obj.message.charAt(0) == "/") {
                    ...
                    if (filter.isProfane(test) || filter.isProfane(test2) || filter.isProfane(test3) || test4 == "") {
                        ...
                        // if (gameState.players[socket.id].avatar == 37) {
                        //   if (Math.random() > 0.66) {
                        //     io.to(obj.room).emit('playerTalked', { id: socket.id, color: obj.color, message: "squeak", x: obj.x, y: obj.y });
                        //   }
                        //   else {
    ...
    socket.on('changeRoom', function (obj) {
        try {
            ...
            if (roomPlayers > MAX_PLAYERS_PER_ROOM && MAX_PLAYERS_PER_ROOM != -1) {
                ...
                if (playerObject.room == "qugddRoomYellow") {
                  console.log("TURN INTO A MOUSE!");
                  io.sockets.emit('playerChanged', { id: socket.id, avatar: 37 });
                  //playerObject.updateCharacter();
                }
    ...
    //when I receive a change sent it to everybody
    socket.on('changeAvatar', function (obj) {
        try {
            gameState.players[socket.id].lastActivity = new Date().getTime();

            //broadcast the movement to everybody
            io.to(obj.room).emit('playerChanged', { id: socket.id, avatar: obj.avatar });
    ...
    try {
        ...
        switch (cmd[0]) {
            ...
            case "rat":
                var s = socketByName(cmd[1]);
                if (s != null) {
                  console.log("calling playerChanged");
                    //s.emit('playerChanged', { id: s.id, avatar: 37 });
                    io.sockets.emit('playerChanged', { id: s.id, avatar: 37 });
                }
                else {
                    //popup to admin
                    adminSocket.emit("popup", "I can't find a user named " + cmd[1]);
...
let myBadWords = ['chink', 'cunt', 'cunts', "fag", "fagging", "faggitt", "faggot", "faggs", "fagot", "fagots", "fags", "jap", "homo", "nigger", "niggers", "n1gger", "nigg3r", "fuck"];
